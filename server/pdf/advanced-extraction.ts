...
const PDF_PARSE_OPTIONS = {
  // Limit pages to be parsed to improve performance (set to 0 for all pages)
  max: 0,
  // Include all text in the output, including headers/footers
  pagerender: undefined
};

/**
 * Extract text from PDF using multiple methods 
 * First tries regular text extraction, then falls back to OCR if needed
 */
export async function extractTextFromPDFAdvanced(pdfBuffer: Buffer): Promise<string> {
  try {
    ...
    try {
      ...
      if (pdfData.text && pdfData.text.length > 200) {
        console.log(`Successfully extracted ${pdfData.text.length} characters with regular method`);
        return pdfData.text;
      }
    ...
    // Fall back to OCR if regular extraction fails or yields too little text
    const ocrText = await extractTextFromPDFWithOCR(pdfBuffer);
    
    if (ocrText && ocrText.length > 100) {
      console.log(`Successfully extracted ${ocrText.length} characters with OCR`);
      return ocrText;
    }
...
/**
 * Advanced metadata interface with more detailed information
 */
...
/**
 * Extract advanced metadata using academic document heuristics
 */
export async function extractAdvancedMetadataFromText(text: string): Promise<AdvancedExtractedMetadata> {
  ...
  try {
    ...
    console.log(`Title found: ${metadata.title ? 'Yes' : 'No'}`);
    console.log(`Authors found: ${metadata.authors.length > 0 ? 'Yes' : 'No'}`);
    console.log(`Abstract found: ${metadata.abstract ? 'Yes' : 'No'}`);
    console.log(`Sections found: ${Object.keys(metadata.sections).length}`);
...
/**
 * Convert advanced metadata to the simpler format
 */
export function convertToBasicMetadata(advanced: AdvancedExtractedMetadata): ExtractedMetadata {
  return {
    title: advanced.title || 'Untitled Document',
    authors: advanced.authors?.join('; ') || 'Unknown Author(s)',
    abstract: advanced.abstract || '',
    keywords: advanced.keywords || [],
    journal: advanced.publicationInfo || '',
